    ...
    setDate(selectedDate){
        ...
        if(list && list.length > 0)  {
            ...
            if(date === selectedDate){
                defaultWeather=date;
                $(`#${selectedDate.date}`).css('background-color','rgb(239,230,230)');
            }
            else{
                $(`#${selectedDate.date}`).css('background-color','rgb(245, 245, 245)');
            }
            });
        ...
        return (
            <Card>
                ...
                <CarouselWrapper showLoader={list.length === 0 ? true : false}>
                    <Carousel>
                        {list.map((day) => <SlidingCard isSelected={this.isSelected(day)} onParticularDate={this.setDate} day={day} key={day.date} id={day.date} />)}
                    </Carousel>
                </CarouselWrapper>
            </Card>
        );
